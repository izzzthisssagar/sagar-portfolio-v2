...
Authentication is single-admin with no registration. Password verification uses Argon2id. The API guard and the shared server-only Next.js verifier cryptographically validate the HS256 signature, expiration, issuer, audience, subject, and `admin` role; absent or placeholder verification configuration fails closed. No bypass token exists in production code. Both the Next.js proxy and authenticated CMS layout verify the access JWT rather than trusting cookie existence; `/admin/login` is outside that layout. Access tokens are short-lived; refresh sessions rotate and store only hashes. Refresh cookies are `HttpOnly`, `Secure` in production, `SameSite=Strict`, and scoped to auth routes. Production credential provisioning and the login/session issuance vertical are deliberately external, so login remains unavailable rather than insecure.

Projects use Prisma/PostgreSQL with validated create/update DTOs, stable pagination, filtering and sorting, conflict responses for duplicate slugs, transactional audit events, and an initial migration. Public `/api/v1/projects` routes expose published records only and resolve details by slug. Guarded `/api/v1/admin/projects` routes expose all states and own CRUD by database ID. Unit tests isolate Prisma behind mocks; the integration suite executes the same service and route boundary against PostgreSQL when `DATABASE_URL` is present.
