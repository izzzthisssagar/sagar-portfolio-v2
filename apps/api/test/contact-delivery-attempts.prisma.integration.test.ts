...
databaseSuite(
  'Contact delivery attempts — reserved before send, one row per real send (real PostgreSQL)',
  () => {
    ...
    it('A. first send succeeds — one SMTP invocation, one attempt row numbered 1, finalized SUCCEEDED', async () => {
      ...
      expect(rows[0]).toMatchObject({ attemptNumber: 1, status: 'SUCCEEDED' });
    });
    ...
    it('B. first send fails, automatic retry succeeds — two invocations, attempts 1 (failed) and 2 (succeeded)', async () => {
      ...
      expect(rows[0]).toMatchObject({ attemptNumber: 1, status: 'FAILED', reason: 'SMTP timeout' });
      expect(rows[1]).toMatchObject({ attemptNumber: 2, status: 'SUCCEEDED' });
    });
    ...
    it('C. both automatic sends fail — two failed attempt rows, numbered 1 and 2', async () => {
      ...
      const rows = await attempts(created.id);
      expect(rows).toHaveLength(2);
      expect(rows[0]).toMatchObject({ attemptNumber: 1, status: 'FAILED' });
      expect(rows[1]).toMatchObject({ attemptNumber: 2, status: 'FAILED' });
    });

    it('G. the attempt row is reserved (PENDING) in the database before the real send resolves', async () => {
      notifier.setFallback({ delivered: true });
      const created = await createMessage('g-reserve-before-send');
      expect(notifier.calls).toHaveLength(1); // the automatic submit-time send, already succeeded

      notifier.reset();
      notifier.setFallback({ delivered: true });
      let duringSend: Awaited<ReturnType<typeof attempts>> | null = null;
      notifier.onSend = async () => {
        // Runs *while* send() is still in flight, before it resolves — proves the row already
        // exists (and is PENDING, not yet finalized) at the moment the real send happens, not
        // sometime after.
        duringSend = await attempts(created.id);
      };

      await contactService.retryNotification(created.id);

      expect(duringSend).not.toBeNull();
      const reservedDuringSend = duringSend!.find((r) => r.attemptNumber === 2);
      expect(reservedDuringSend).toBeDefined();
      expect(reservedDuringSend?.status).toBe('PENDING');

      // Finalized to SUCCEEDED once the send (and this whole retry) completes.
      const rows = await attempts(created.id);
      expect(rows.find((r) => r.attemptNumber === 2)?.status).toBe('SUCCEEDED');
    });
    ...
    it('F. a failed finalize write is never reported as success, and never leaves a phantom attempt row', async () => {
      const created = await createMessage('f-db-write-fails');
      expect(notifier.calls).toHaveLength(1); // the real submit-time send

      notifier.reset();
      notifier.setFallback({ delivered: true });
      // The attempt row was already reserved (PENDING) *before* this send was invoked (see
      // reserveAttempt / scenario G) — this simulates an admin concurrently deleting the message
      // while the real send is still in flight, so finalizeAttempt's own UPDATE (which runs after
      // send() resolves) hits a row that no longer exists: a genuine "record not found" failure,
      // not a simulated/mocked Prisma failure. ContactDeliveryAttempt.contactMessageId has
      // onDelete: Cascade, so both the pre-existing attempt #1 row and the #2 reservation are
      // genuinely gone once this fires — a real, observable consequence of the message no longer
      // existing, not a bug in this test.
      notifier.onSend = async () => {
      ...
      expect(notifier.calls).toHaveLength(1);
      // But nothing about it is durably, falsely recorded as a success: the whole finalize
      // transaction (finalize UPDATE + claim-clear + audit) rolled back when the UPDATE hit a
      // deleted row. The resulting state is honestly inspectable (an admin investigating sees the
      // message is simply gone, not a phantom "attempt 2" row pointing at nothing) — a documented,
      // recoverable state, not a silently-swallowed lie or an uncontrolled retry loop (there is
